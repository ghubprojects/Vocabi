...
namespace Vocabi.Infrastructure.External.Pixabay;

public class PixabayProvider(HttpClient httpClient, IConfiguration config) : IImageProvider
{
    private readonly string _apiKey = config["Pixabay:ApiKey"] ?? throw new ArgumentNullException("Pixabay API key not configured");

    public string ProviderName => "Pixabay API";

    public async Task<Result<IReadOnlyList<string>>> SearchAsync(string keyword, int limit = 5, CancellationToken cancellationToken = default)
    {
